...
- **Distribution parameters** let you tune normal/Poisson histograms, Monte Carlo π domain size, plus per-simulation knobs such as worm length, Snake starting length/pause, the Snake "wrap edges" toggle, the new `snake_block_count` obstacle count, and the Pac-Man ghost count.
