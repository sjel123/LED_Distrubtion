...
- Snake '97 exposes a "Allow Snake to wrap edges" checkbox so you can opt into wrapping/move beyond the panel before resets; leave it unchecked for wall collisions only.
- The Snake mode also accepts a `snake_block_count` value; it drops that many static white obstacles onto the grid for the snake to avoid (set to `0` to disable).
